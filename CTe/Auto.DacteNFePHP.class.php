...
require_once '..\\..' . DIRECTORY_SEPARATOR . 'libs' . DIRECTORY_SEPARATOR . 'PdfNFePHP.class.php';
require_once '..\\..' . DIRECTORY_SEPARATOR . 'libs' . DIRECTORY_SEPARATOR . 'CommonNFePHP.class.php';
require_once '..\\..' . DIRECTORY_SEPARATOR . 'libs' . DIRECTORY_SEPARATOR . 'DocumentoNFePHP.interface.php';
...
class DacteNFePHP extends CommonNFePHP implements DocumentoNFePHP
{
    ...
    public function __construct(
        ...
        $this->formatPadrao = array('font' => $this->fontePadrao, 'size' => 7,
            'style' => '');
        $this->formatNegrito = array('font' => $this->fontePadrao, 'size' => 8,
            'style' => 'B');
        ...
        if (!empty($this->xml)) {
            ...
            $this->protCTe = $this->dom->getElementsByTagName("protCTe")
                ->item(0);
        }
    } //fim construct
	
	
	public function abrevia($nome,$qtd) {
		$nome = reset(explode("-",$nome));
		$nome = trim($nome);

		for($i=0;$i<10;$i++) {
			$pos = strpos($nome,$i.'');
			if (!$pos === false) { $nome = substr($nome, 0, ($pos-1)); break; }
		}
		$nome = trim($nome);
		...
		if (strlen($nome)>$qtd) {
			$aux = explode(" ",$nome);
			if(count($aux)>2) {
				$primeiro = array_shift($aux);
				$ultimo = array_pop($aux);
				$txt = '';
				$i=0;
				foreach($aux as $value) {
					$txt .= $value[0]." ";
				}
				$abreviado = $primeiro." ".$txt.$ultimo;
				if (strlen($abreviado)>$qtd) {
					return substr($abreviado, 0, $qtd); 
				} else {
					return $abreviado;
				}
			} else {
				return substr($nome, 0, $qtd); 
			}
		} else {
			return $nome;
		}
    ...
    {
        if (1 == 2 || $this->xml == null || $this->infCte == null ||
            $this->ide == null) {
            return false;
        }
    ...
    protected function cabecalhoDACTE($x = 0, $y = 0, $pag = '1', $totPag = '1')
    {
        ...
        if (is_file($this->logomarca)) {
            ...
            $this->pdf->Image($this->logomarca, $xImg, ($yImg-2), $nImgW, $nImgH, 'jpeg');
        } else {
            $x1 = $x;
            $y1 = round($h / 3 + $y, 0);
            $tw = $w;
        ...
        $aFont = array('font' => $this->fontePadrao, 'size' => 8, 'style' => '');
        $this->__textBox($xa, $y + 1, $wa, $h, $texto, $aFont, 'T', 'C', 0, '');
        $texto = !empty($this->ide->getElementsByTagName("dhEmi")
                ->item(0)->nodeValue) ? date(
                    'd/m/Y H:i:s',
                    $this->__convertTime($this->__simpleGetValue($this->ide, "dhEmi"))
                ) : '';
        $aFont = $this->formatNegrito;
        $this->__textBox($xa, $y + 5, $wa, $h, $texto, $aFont, 'T', 'C', 0, '');
        $this->pdf->Line($xa + $wa, $y, $xa + $wa, $y + $h + 1);
        ...
        if($this->__cteDPEC()){
	     ...
	     $cStat = '';
            ...
            if (isset($this->cteProc)) {
                $texto = !empty($this->cteProc->getElementsByTagName("nProt")->item(0)
                        ->nodeValue) ? $this->cteProc->getElementsByTagName("nProt")
                        ->item(0)->nodeValue : '';
...
*/
    ...
    protected function remetenteDACTE($x = 0, $y = 0)
    {
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $texto = $this->__format(
            $this->__simpleGetValue($this->enderReme, "CEP"),
            "#####-###"
        );
        $aFont = $this->formatNegrito;
    ...
    protected function destinatarioDACTE($x = 0, $y = 0)
    {
        ...
        $aFont = $this->formatNegrito;
        //$texto = $this->abrevia($this->__simpleGetValue($this->dest, "xNome"),45);
        $texto = substr($this->__simpleGetValue($this->dest, "xNome"),0,45);
        $this->__textBox($x1, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $y += 3;
        $texto = 'ENDEREÇO';
        $aFont = $this->formatPadrao;
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $texto = $this->__format(
            $this->__simpleGetValue($this->enderDest, "CEP"),
            "#####-###"
        );
        $aFont = $this->formatNegrito;
    ...
    protected function expedidorDACTE($x = 0, $y = 0)
    {
        ...
        if (isset($this->enderExped)) {
            ...
            $texto .= $this->__simpleGetValue($this->enderExped, "xCpl") != "" ?
                    ' - ' . $this->__simpleGetValue($this->enderExped, "xCpl") : '';
        } else {
            $texto = '';
        }
        $this->__textBox($x1, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $texto = $this->__format(
            $this->__simpleGetValue($this->enderExped, "CEP"),
            "#####-###"
        );
        $aFont = $this->formatNegrito;
        $this->__textBox($x + 6, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
    ...
    protected function recebedorDACTE($x = 0, $y = 0)
    {
        ...
        if (isset($this->enderReceb)) {
            ...
            $texto .= ($this->__simpleGetValue($this->enderReceb, "xCpl") != "") ?
                    ' - ' . $this->__simpleGetValue($this->enderReceb, "xCpl") : '';
        } else {
            $texto = '';
        }
        $this->__textBox($x1, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $texto = $this->__format(
            $this->__simpleGetValue($this->enderReceb, "CEP"),
            "#####-###"
        );
        $aFont = $this->formatNegrito;
        $this->__textBox($x + 5, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
    ...
    protected function tomadorDACTE($x = 0, $y = 0)
    {
        ...
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        $texto = $this->__format(
            $this->__simpleGetValue($this->toma, "CEP"),
            "#####-###"
        );
        $aFont = $this->formatNegrito;
        $this->__textBox($x + 6, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
    ...
    protected function descricaoCargaDACTE($x = 0, $y = 0)
    {
        ...
        $texto .= number_format(
            $this->__simpleGetValue($this->infQ->item(0), "qCarga") /
            $this->multiUniPeso($this->__simpleGetValue($this->infQ->item(0), "cUnid")),
            3,
            ",",
            "."
        );
        ...
        $texto .= number_format(
            $this->__simpleGetValue($this->infQ->item(1), "qCarga") /
            $this->multiUniPeso($this->__simpleGetValue($this->infQ->item(1), "cUnid")),
            3,
            ",",
            "."
        );
        ...
        $texto .= number_format(
            $this->__simpleGetValue($this->infQ->item(2), "qCarga") /
            $this->multiUniPeso($this->__simpleGetValue($this->infQ->item(2), "cUnid")),
            3,
            ",",
            "."
        );
    ...
    protected function obsDACTE($x = 0, $y = 0)
    {
        ...
        $aFont = array('font' => $this->fontePadrao, 'size' => 7.5, 'style' => '');
    ...
    protected function modalRodDACTE($x = 0, $y = 0)
    {
        ...
        if ($this->lota == 1) {
            ...
            $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
            $this->pdf->Line($x, $y, $w + 1, $y);
            $texto = !empty($this->moto) ? $this->abrevia($this->__simpleGetValue($this->moto, "xNome"),25) : '';
            $aFont = array('font' => $this->fontePadrao, 'size' => 7, 'style' => 'B');
    ...
    protected function canhotoDACTE($x = 0, $y = 0)
    {
		...
		// TESTESTESTESTESTE
		//$this->infNF->item($k)->getElementsByTagName('nDoc')->item(0)->nodeValue;
        $texto .= "\r\n NF " . $this->infNF->item(0)->getElementsByTagName('nDoc')->item(0)->nodeValue;
        $aFont = array('font' => $this->fontePadrao, 'size' => 6, 'style' => '');
