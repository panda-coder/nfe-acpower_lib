...
require_once('..\\..' . DIRECTORY_SEPARATOR . 'libs' . DIRECTORY_SEPARATOR . 'DacteNFePHP.class.php');
...
class AutoCTeSyslog extends CTeNFePHP {
	...
	public function recuperaProtocol($id){
		...
		try{
			...
			$this->errMsg = "Erro ao recuperar Protocolo - ".$e;
			$this->errStatus = true;
		}
	}
	
	public function autoCancel($id_ctesyslog, $justificativa) {
		$resp = '';
		$ctesyslog = new CteSyslog_x_bd($this->DbConnection, $id_ctesyslog);
		
		if($ctesyslog->chave_cte!='') {
			
			$chaveCTe = $ctesyslog->chave_cte;
			
			$xml_nome = $chaveCTe.'-cte.xml';
			$pdf_nome = $chaveCTe.'.pdf';
			$ano_mes = '20'.substr($xml_nome,2,4);
			$aprFile = $this->aprDir.$ano_mes.DIRECTORY_SEPARATOR.$xml_nome;
			$pdfFile = $this->pdfDir.$ano_mes.DIRECTORY_SEPARATOR.$pdf_nome;
			
			
			if($retCancel = $this->cancelEvent($chaveCTe, $ctesyslog->getProtocol(), $this->modSOAP, $resp)){
				// obteve retorno
				if($resp['bStat']=='1') {
				
					$protFile = $this->evtDir.$chaveCTe.'-110111-1-procCTe.xml';
					// retorno positivo
					if($resp['cStat']=='135') {
					
						// verifica se possui um protocolo de evento
						if (file_exists($protFile)) {
							
							// copia o protocolo para o diretorio de cancelados
							copy($protFile, $this->canDir.$chaveCTe.'-110111-1-procCTe.xml');
							
							// cria o arquivo no diretorio aprovados se o mesmo nao existe
							if(!file_exists($aprFile)) { file_put_contents($aprFile, $ctesyslog->getXML()); }
							
							// Move o CTe com o protocolo na pasta cancelados
							if(file_exists($aprFile)) {
								rename($aprFile, $this->canDir.$chaveCTe.'-cte.xml');
							}
						...
						$ctesyslog->saveLog(utf8_encode($resp['xMotivo']));
						$ctesyslog->saveCancelada();
					} else {
						$ctesyslog->saveLog(utf8_encode($resp['xMotivo']));
					}
