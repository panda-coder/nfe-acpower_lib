...
function procNFe($v_NFe,$xml,$nfe,$manif) {
	global $db_nfe;
...
function procCanc($v_Canc,$xml,$nfe,$manif) {
	global $db_nfe;
	global $CNPJ_EMITENTE;
...
function procCCe($v_CCE,$xml,$nfe,$manif) {
	global $db_nfe;
	global $CNPJ_EMITENTE;
...
function getListNFe_Bd($nfe) {
	global $AN,$tpAmb,$modSOAP,$db_nfe,$CNPJ_EMITENTE;
	$arr_nt = array();
	if ($xml = $nfe->getListNFe($AN, $indNFe, $indEmi, '', $tpAmb, $modSOAP, $arr_nt)){
		
		$res_manif = $db_nfe->lookup("select manifesta_ciencia from t_parametros where id_empresa in (select id_empresa from t_empresas where cnpj = '".$CNPJ_EMITENTE."')");
		...
		*/
		...
		$it_NSU = '';
		if(count($v_NFe)>0)  { $it_NSU = procNFe($v_NFe,$xml,$nfe,$res_manif[0][0]); }
		if(count($v_Canc)>0) { $it_NSU = procCanc($v_Canc,$xml,$nfe,$res_manif[0][0]); }
		if(count($v_CCE)>0)  { $it_NSU = procCCe($v_CCE,$xml,$nfe,$res_manif[0][0]); }
	}
...
function initGetList($nfe) {
	...
	while(1) {
		if (!getListNFe_Bd($nfe)) { sleep(1800); } else { sleep(3); }
	}
}
